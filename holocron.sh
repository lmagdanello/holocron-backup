...
Backup() {
    ...
    for type in ${TYPES}; do
        for server in $(yq r ${CONFIG} --printMode p "servers.type.${type}.*.*" | awk -F '.' '{print $NF}' | uniq | xargs); do
            for source in $(yq r ${CONFIG} --printMode pv "servers.type.${type}.*.${server}" | awk -F ':' '{print $NF}' | sed 's/-/ /g' | uniq | xargs); do
                ssh ${server} sudo tar czfP - /${source} >${BACKUPDIR}/${type}/$(echo ${source} | awk -F '/' '{print $NF}')_${server}_$(date --iso-8601).tar.gz
